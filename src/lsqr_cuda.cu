                               ...
                               const double t3)
{
    ...
    *dknorm_vec = 0.0;
    ...
    if (lane == 0)
        val[wid] = sum; // Write reduced value to shared memory
    __syncthreads();
    ...
    struct comData comlsqr)
{
...
#ifndef USE_MPI
    std::cout << "nAttParAxis = " << nAttParAxis << std::endl;
    std::cout << "PE = " << myid << " nnz = " << nnz << std::endl;
    std::cout << "THREADS_PER_COL = ThreadsXBlockAprod2Astro = " << ThreadsXBlockAprod2Astro << std::endl; // ONLY FOR CUDA AND HIP
#endif
    ...
    checkCuda(cudaMemcpy(&max_knownTerms, dev_max_knownTerms, sizeof(double), cudaMemcpyDeviceToHost));
    ...
    if (myid == 0)
    {
        ...
        cudaMemcpyAsync(&ssq_knownTerms, dev_ssq_knownTerms, sizeof(double), cudaMemcpyDeviceToHost, stream);
    ...
    if (beta > ZERO)
    {
        ...
        if (nAstroPSolved)
            aprod2_Kernel_astro<RowsPerBlock, ThreadsXBlockAprod2Astro><<<gridAprod2astro, blockAprod2astro, 0, streamAprod2_0>>>(vVect_dev, sysmatAstro_dev, knownTerms_dev, matrixIndexAstro_dev, startend_dev, offLocalAstro, nnz, nAstroPSolved);
        if (nAttP)
            aprod2_Kernel_att_AttAxis<<<gridDim_aprod2attinstr, ThreadsXBlockAprod2AttInstr, 0, streamAprod2_1>>>(vVect_dev, sysmatAtt_dev, knownTerms_dev, matrixIndexAtt_dev, nAttP, nDegFreedomAtt, offLocalAtt, mapNoss, nAstroPSolved, nAttParAxis);
        if (nInstrPSolved)
            aprod2_Kernel_instr<<<gridDim_aprod2attinstr, ThreadsXBlockAprod2AttInstr, 0, streamAprod2_2>>>(vVect_dev, sysmatInstr_dev, knownTerms_dev, instrCol_dev, offLocalInstr, mapNoss, nInstrPSolved);
        for (short inGlob = 0; inGlob < nGlobP; inGlob++)
        {
        ...
        //  CONSTRAINTS OF APROD MODE 2:
        if (nEqExtConstr)
            aprod2_Kernel_ExtConstr<<<gridDim_aprod2_ExtConstr, TILE_WIDTH, 0, streamAprod2_5>>>(vVect_dev, sysmatInstr_dev, knownTerms_dev, mapNoss, nDegFreedomAtt, VrIdAstroPDimMax, nEqExtConstr, nOfElextObs, numOfExtStar, startingAttColExtConstr, numOfExtAttCol, nAttAxes, nAstroPSolved);
        if (nEqBarConstr)
            aprod2_Kernel_BarConstr<<<gridDim_aprod2_BarConstr, TILE_WIDTH, 0, streamAprod2_6>>>(vVect_dev, sysmatConstr_dev, knownTerms_dev, mapNoss, nEqBarConstr, nEqExtConstr, nOfElextObs, nOfElBarObs, numOfBarStar, nAstroPSolved);
        if (nOfInstrConstr)
...
//------------------------------------------------------------------------------------------------  TIME6
    ...
    if (alpha > ZERO)
    {
        dscal<<<gridSize, blockSize, 0, stream>>>(vVect_dev, 1 / alpha, nunkSplit, 1.0);
...
//{ // CONTEXT MODE 1//////////////////////////////////// APROD MODE 1
        ...
        if (nAttP)
            aprod1_Kernel_att_AttAxis<<<gridDim_aprod1attinstr, ThreadsXBlockAprod1AttInstr, 0, stream>>>(knownTerms_dev, sysmatAtt_dev, vVect_dev, matrixIndexAtt_dev, nAttP, mapNoss, nDegFreedomAtt, offLocalAtt, nAttParAxis);
        ...
        if (nInstrPSolved)
            aprod1_Kernel_instr<<<gridDim_aprod1attinstr, ThreadsXBlockAprod1AttInstr, 0, stream>>>(knownTerms_dev, sysmatInstr_dev, vVect_dev, instrCol_dev, mapNoss, offLocalInstr, nInstrPSolved);
        ...
        if (nGlobP)
            aprod1_Kernel_glob<<<gridDim_aprod1, TILE_WIDTH, 0, stream>>>(knownTerms_dev, sysmatGloB_dev, vVect_dev, offLocalGlob, mapNoss, nGlobP);
        // //        CONSTRAINTS APROD MODE 1
        if (nEqExtConstr)
            aprod1_Kernel_ExtConstr<<<gridDim_aprod1_ExtConstr, TILE_WIDTH, 0, stream>>>(knownTerms_dev, sysmatConstr_dev, vVect_dev, VrIdAstroPDimMax, mapNoss, nDegFreedomAtt, startingAttColExtConstr, nEqExtConstr, nOfElextObs, numOfExtStar, numOfExtAttCol, nAstroPSolved, nAttAxes);
        if (nEqBarConstr)
            aprod1_Kernel_BarConstr<<<gridDim_aprod1_BarConstr, TILE_WIDTH, 0, stream>>>(knownTerms_dev, sysmatConstr_dev, vVect_dev, nOfElextObs, nOfElBarObs, nEqExtConstr, mapNoss, nEqBarConstr, numOfBarStar, nAstroPSolved);
        if (nOfInstrConstr)
        ...
        {
            sumCommMultiBlock_double<blockSize><<<gridSize, blockSize>>>(knownTerms_dev, dev_ssq_knownTerms, max_knownTerms, mapNoss);
            realsumCommMultiBlock_double<blockSize><<<1, blockSize>>>(dev_ssq_knownTerms, dev_ssq_knownTerms, gridSize);
            checkCuda(cudaMemcpyAsync(&ssq_knownTerms, dev_ssq_knownTerms, sizeof(double), cudaMemcpyDeviceToHost, stream));
            cudaStreamSynchronize(stream);
            betaLoc = max_knownTerms * sqrt(ssq_knownTerms);
            betaLoc2 = betaLoc * betaLoc;
...
//------------------------------------------------------------------------------------------------
        ...
        if (beta > ZERO)
        {
...
#ifdef KERNELTIME
            milliseconds = 0.0;
            cudaStreamSynchronize(streamAprod2_0);
            ...
            milliseconds = 0.0;
            cudaStreamSynchronize(streamAprod2_1);
            ...
            milliseconds = 0.0;
            cudaStreamSynchronize(streamAprod2_2);
            ...
            {
                double alphaOther2 = alphaLoc2;
                sumCommMultiBlock_double<blockSize><<<gridSize, blockSize>>>(&vVect_dev[localAstroMax], dev_ssq_vVect, max_vVect, nunkSplit - localAstroMax);
            ...
            if (alpha > ZERO)
            {
                dscal<<<gridSize, blockSize, 0, stream>>>(vVect_dev, 1 / alpha, nunkSplit, 1);
            }
        }
...
#ifdef USE_MPI
        checkCuda(cudaMemcpyAsync(&dknorm, dknorm_vec, sizeof(double), cudaMemcpyDeviceToHost, stream));
        //------------------------------------------------------------------------------------------------
        starttime = get_time();
        ...
        communicationtime += compute_time(endtime, starttime);
        //------------------------------------------------------------------------------------------------
        checkCuda(cudaMemcpyAsync(dknorm_vec, &dknorm, sizeof(double), cudaMemcpyHostToDevice, stream));
#else
        cudaStreamSynchronize(stream);
#endif
        ...
        if (wantse)
            transform2<<<BlockXGrid, ThreadsXBlock, 0, stream>>>(standardError_dev, wVect_dev, 0, localAstro, t3);
        ...
        if (wantse)
            transform2<<<BlockXGrid, ThreadsXBlock, 0, stream>>>(standardError_dev, wVect_dev, localAstroMax, localAstroMax + other, t3);
