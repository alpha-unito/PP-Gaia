...
#include <atomic>
#include <CL/sycl.hpp>
#define atomicAdd(x, y) (cl::sycl::atomic_ref<double, cl::sycl::memory_order::relaxed, cl::sycl::memory_scope::device, cl::sycl::access::address_space::generic_space>(*(x)) += (y))
#endif
                               ...
                               const short nInstrPSolved)
{
    ...
    if (myid < nOfInstrConstr)
    {
        ...
        std::transform(POL, start, end, iter.begin(), iter.begin(), [nproc](auto x, auto y)
                       { return y + nproc * x; });
...
//------------------------------------------------------------------------------------------------
    ...
    if (beta > ZERO)
    {
        ...
        if (nAstroPSolved)
            aprod2_Kernel_astro(vVect, sysmatAstro, knownTerms, matrixIndexAstro, startend, offLocalAstro, nnz, nAstroPSolved);
        if (nAttP)
            aprod2_Kernel_att_AttAxis(vVect, sysmatAtt, knownTerms, matrixIndexAtt, nAttP, nDegFreedomAtt, offLocalAtt, mapNoss, nAstroPSolved, nAttParAxis);
        if (nInstrPSolved)
            aprod2_Kernel_instr(vVect, sysmatInstr, knownTerms, instrCol, offLocalInstr, mapNoss, nInstrPSolved);
        // APROD2 CONSTRAINTS
        if (nEqExtConstr)
            aprod2_Kernel_ExtConstr(vVect, sysmatConstr, knownTerms, mapNoss, nDegFreedomAtt, VrIdAstroPDimMax, nEqExtConstr, nOfElextObs, numOfExtStar, startingAttColExtConstr, numOfExtAttCol, nAstroPSolved, nAttAxes);
        if (nEqBarConstr)
            aprod2_Kernel_BarConstr(vVect, sysmatConstr, knownTerms, mapNoss, nEqBarConstr, nEqExtConstr, nOfElextObs, nOfElBarObs, numOfBarStar, nAstroPSolved);
        if (nOfInstrConstr)
            aprod2_Kernel_InstrConstr(vVect, sysmatConstr, knownTerms, instrConstrIlung, instrCol, VrIdAstroPDimMax, nDegFreedomAtt, mapNoss, nEqExtConstr, nEqBarConstr, nOfElextObs, nOfElBarObs, myid, nOfInstrConstr, nproc, nInstrPSolved, nAstroPSolved, nAttAxes);
...
// //////////////////////////////////// APROD MODE 1
        ...
        if (nAstroPSolved)
            aprod1_Kernel_astro(knownTerms, sysmatAstro, vVect, matrixIndexAstro, offLocalAstro, mapNoss, nAstroPSolved);
        ...
        if (nAttP)
            aprod1_Kernel_att_AttAxis(knownTerms, sysmatAtt, vVect, matrixIndexAtt, nAttP, mapNoss, nDegFreedomAtt, offLocalAtt, nAttParAxis);
        ...
        if (nInstrPSolved)
            aprod1_Kernel_instr(knownTerms, sysmatInstr, vVect, instrCol, mapNoss, offLocalInstr, nInstrPSolved);
        ...
        if (nGlobP)
            aprod1_Kernel_glob(knownTerms, sysmatGloB, vVect, offLocalGlob, mapNoss, nGlobP);
        // //////////////////////////////////// CONSTRAi_intS APROD MODE 1
        if (nEqExtConstr)
            aprod1_Kernel_ExtConstr(knownTerms, sysmatConstr, vVect, VrIdAstroPDimMax, mapNoss, nDegFreedomAtt, startingAttColExtConstr, nEqExtConstr, nOfElextObs, numOfExtStar, numOfExtAttCol, nAstroPSolved, nAttAxes);
        if (nEqBarConstr)
            aprod1_Kernel_BarConstr(knownTerms, sysmatConstr, vVect, nOfElextObs, nOfElBarObs, nEqExtConstr, mapNoss, nEqBarConstr, numOfBarStar, nAstroPSolved);
        if (nOfInstrConstr)
            aprod1_Kernel_InstrConstr(knownTerms, sysmatConstr, vVect, instrConstrIlung, instrCol, VrIdAstroPDimMax, mapNoss, nDegFreedomAtt, nOfElextObs, nEqExtConstr, nOfElBarObs, nEqBarConstr, myid, nOfInstrConstr, nproc, nAstroPSolved, nAttAxes, nInstrPSolved);
        ...
        dknorm = std::transform_reduce(POL, wVect, wVect + nAstroElements * nAstroPSolved, ZERO, std::plus<>(), [=](const auto &x)
                                       { return (t3 * x) * (t3 * x); });
        ...
        dknorm = std::transform_reduce(POL, wVect + localAstroMax, wVect + localAstroMax + other, dknorm, std::plus<>(), [=](const auto &x)
                                       { return (t3 * x) * (t3 * x); });
