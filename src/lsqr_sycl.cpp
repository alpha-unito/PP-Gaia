...
#define atomicAdd(x, y) (sycl::atomic_ref<double, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::generic_space>(*(x)) += (y))
    ...
    struct comData comlsqr)
{
    ...
    if (wantse)
        dload(nunkSplit, ZERO, standardError);
    ...
    if (arnorm == ZERO)
    {
        ...
        if (nAstroPSolved)
            sycl::free(sysmatAstro_dev, queue);
        if (nAttP)
            sycl::free(sysmatAtt_dev, queue);
        if (nInstrPSolved)
            sycl::free(sysmatInstr_dev, queue);
        if (nGlobP)
            sycl::free(sysmatGloB_dev, queue);
        if (nTotConstraints)
            sycl::free(sysmatConstr_dev, queue);
...
//{ // CONTEXT MODE 1//////////////////////////////////// APROD MODE 1
        ...
        {
            queue.submit([&](sycl::handler &cgh)
                         {double *knownTerms_dev_mapNoss_ct0 = &knownTerms_dev[mapNoss];
