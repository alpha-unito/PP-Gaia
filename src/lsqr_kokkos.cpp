...
inline void dscal(Kokkos::View<K> array, const double val, const long N, const double sign)
{
    ...
    Kokkos::parallel_for("dscal", N, KOKKOS_LAMBDA(const long i) { array[i] = sign * (array[i] * val); });
}
...
inline double maxCommMultiBlock_double(Kokkos::View<K> gArr, const L arraySize)
{
    ...
    Kokkos::parallel_reduce("max_reduce", arraySize, KOKKOS_LAMBDA(const L i, double &local_max) {
        if(fabs(gArr[i])>local_max) local_max=fabs(gArr[i]); }, Kokkos::Max<double>(max));
...
inline double sumCommMultiBlock_double(Kokkos::View<K> gArr, const L arraySize, const double max)
{
    ...
    Kokkos::parallel_reduce("sumCommMultiBlock_double", arraySize, KOKKOS_LAMBDA(const L i, double &locsum) { locsum += (gArr[i] * d) * (gArr[i] * d); }, sum);
    return sum;
}
...
inline double sumCommMultiBlock_double_start(Kokkos::View<K> gArr, const L arraySize, const double max, const L start)
{
    ...
    Kokkos::parallel_reduce("sumCommMultiBlock_double_start", arraySize, KOKKOS_LAMBDA(const L i, double &locsum) { locsum += (gArr[start + i] * d) * (gArr[start + i] * d); }, sum);
    return sum;
}
...
void vVect_Put_To_Zero_Kernel(Kokkos::View<K> vVect_dev, const L localAstroMax, const L nunkSplit)
{
    ...
    Kokkos::parallel_for("vVect_Put_To_Zero_Kernel", Kokkos::RangePolicy<>(localAstroMax, nunkSplit), KOKKOS_LAMBDA(const L i) { vVect_dev[i] = ZERO; });
}
...
void cblas_dcopy_kernel(Kokkos::View<K> wVect_dev, const Kokkos::View<K> vVect_dev, const long nunkSplit)
{
    ...
    Kokkos::parallel_for("cblas_dcopy_kernel", nunkSplit, KOKKOS_LAMBDA(const long i) { wVect_dev[i] = vVect_dev[i]; });
}
...
void kauxsum(Kokkos::View<K> knownTerms_dev, const Kokkos::View<K> kAuxcopy_dev, const L start, const I N)
{
    ...
    Kokkos::parallel_for("kauxsum", N, KOKKOS_LAMBDA(const long ix) { knownTerms_dev[start + ix] = knownTerms_dev[start + ix] + kAuxcopy_dev[ix]; });
}
...
void vaux_sum(Kokkos::View<K> vVect_dev, const Kokkos::View<K> vAuxVect_dev, const L localAstroMax)
{
    ...
    Kokkos::parallel_for("vaux_sum", localAstroMax, KOKKOS_LAMBDA(const L ix) { vVect_dev[ix] += vAuxVect_dev[ix]; });
}
...
//------------------------------------------------------------------------------------------------  TIME 2
        ...
        if (beta > ZERO)
        {
            ...
            if (nAstroPSolved)
                aprod2_Kernel_astro(vVect_dev, sysmatAstro_dev, knownTerms_dev, matrixIndexAstro_dev, startend_dev, offLocalAstro, nnz, nAstroPSolved);
            if (nAttP)
                aprod2_Kernel_att_AttAxis(vVect_dev, sysmatAtt_dev, knownTerms_dev, matrixIndexAtt_dev, nAttP, nDegFreedomAtt, offLocalAtt, mapNoss, nAstroPSolved, nAttParAxis);
            if (nInstrPSolved)
                aprod2_Kernel_instr(vVect_dev, sysmatInstr_dev, knownTerms_dev, instrCol_dev, offLocalInstr, mapNoss, nInstrPSolved);
            // NOT OPTIMIZED YET
            if (nEqExtConstr)
                aprod2_Kernel_ExtConstr(vVect_dev, sysmatConstr_dev, knownTerms_dev, mapNoss, nDegFreedomAtt, VrIdAstroPDimMax, nEqExtConstr, nOfElextObs, numOfExtStar, startingAttColExtConstr, numOfExtAttCol, nAstroPSolved, nAttAxes);
            if (nEqBarConstr)
                aprod2_Kernel_BarConstr(vVect_dev, sysmatConstr_dev, knownTerms_dev, mapNoss, nEqBarConstr, nEqExtConstr, nOfElextObs, nOfElBarObs, numOfBarStar, nAstroPSolved);
            if (nOfInstrConstr)
                aprod2_Kernel_InstrConstr(vVect_dev, sysmatConstr_dev, knownTerms_dev, instrConstrIlung_dev, instrCol_dev, VrIdAstroPDimMax, nDegFreedomAtt, mapNoss, nEqExtConstr, nEqBarConstr, nOfElextObs, nOfElBarObs, myid, nOfInstrConstr, nproc, nInstrPSolved, nAstroPSolved, nAttAxes);
...
////////////////////////  START ITERATIONS
        ...
        while (1)
        {
            ...
            if (nAstroPSolved)
                aprod1_Kernel_astro(knownTerms_dev, sysmatAstro_dev, vVect_dev, matrixIndexAstro_dev, offLocalAstro, mapNoss, nAstroPSolved);
            ...
            if (nAttP)
                aprod1_Kernel_att_AttAxis(knownTerms_dev, sysmatAtt_dev, vVect_dev, matrixIndexAtt_dev, nAttP, mapNoss, nDegFreedomAtt, offLocalAtt, nAttParAxis);
            ...
            if (nInstrPSolved)
                aprod1_Kernel_instr(knownTerms_dev, sysmatInstr_dev, vVect_dev, instrCol_dev, mapNoss, offLocalInstr, nInstrPSolved);
            ...
            if (nGlobP)
                aprod1_Kernel_glob(knownTerms_dev, sysmatGloB_dev, vVect_dev, offLocalGlob, mapNoss, nGlobP);
            // //////////////////////////////////// CONSTRAINTS APROD MODE 1
            if (nEqExtConstr)
                aprod1_Kernel_ExtConstr(knownTerms_dev, sysmatConstr_dev, vVect_dev, VrIdAstroPDimMax, mapNoss, nDegFreedomAtt, startingAttColExtConstr, nEqExtConstr, nOfElextObs, numOfExtStar, numOfExtAttCol, nAstroPSolved, nAttAxes);
            if (nEqBarConstr)
                aprod1_Kernel_BarConstr(knownTerms_dev, sysmatConstr_dev, vVect_dev, nOfElextObs, nOfElBarObs, nEqExtConstr, mapNoss, nEqBarConstr, numOfBarStar, nAstroPSolved);
            if (nOfInstrConstr)
                aprod1_Kernel_InstrConstr(knownTerms_dev, sysmatConstr_dev, vVect_dev, instrConstrIlung_dev, instrCol_dev, VrIdAstroPDimMax, mapNoss, nDegFreedomAtt, nOfElextObs, nEqExtConstr, nOfElBarObs, nEqBarConstr, myid, nOfInstrConstr, nproc, nAstroPSolved, nAttAxes, nInstrPSolved);
            ...
            Kokkos::parallel_for("transform_wVect_xSolution", localAstro, KOKKOS_LAMBDA(const int i) { xSolution_dev(i) = t1 * wVect_dev(i) + xSolution_dev(i); });

            if (wantse)
            {
                Kokkos::parallel_for("transform_wVect_standardError", localAstro, KOKKOS_LAMBDA(const int i) { standardError_dev(i) = standardError_dev(i) + (t3 * wVect_dev(i)) * (t3 * wVect_dev(i)); });
            }

            // Third transformation
            Kokkos::parallel_for("transform_vVect_wVect", localAstro, KOKKOS_LAMBDA(const int i) { wVect_dev(i) = vVect_dev(i) + t2 * wVect_dev(i); });
            ...
            Kokkos::parallel_reduce("transform_reduce_dknorm", other, KOKKOS_LAMBDA(const int i, double &lsum) {
                const int index = localAstroMax + i;
                lsum += (t3 * wVect_dev(index)) * (t3 * wVect_dev(index)); }, dknorm);
