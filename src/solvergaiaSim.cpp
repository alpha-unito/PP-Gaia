 ...
 */
 ...
 int main(int argc, char **argv)
 {
     ...
     // seconds[0] = time(NULL);
     time_point initialTime=get_time();
     ...
     #else
         print_cmdline_input(nfileProc, itnCPR, zeroAtt, zeroInstr, zeroGlob, numberOfCovEle);
         printf("\nProcess %d running \n", system_params::myid);
     #endif
     ...
     #ifdef USE_MPI
         ACTION_IF_RANK_0(system_params::myid, print_params(numberOfCovEle));
     #else
     ...
     if ((att_params::nDegFreedomAtt == 0 && att_params::nAttAxes > 0) ||    (att_params::nDegFreedomAtt > 0 && att_params::nAttAxes == 0))
     {
         if (system_params::myid == 0)
         {
             printf("inconsistent values for nDegFreedomAtt=%ld and nAttaxes=%d\n", att_params::nDegFreedomAtt,att_params::nAttAxes);
             abort();
         }
     }
     ...
     if (constr_params::extConstraint)   extNobxStarfile(mapNoss);
     else if (constr_params::barConstraint)  barNobxStarfile(mapNoss);
     ...
     auto sysmat_start_time = get_time();
     ...
     // for(auto i=0; i<system_params::nElements; ++i){
     //     outfile << lsqr_arrs::knownTerms[i] << std::endl;
     // }
     // outfile.close();
     ...
     if (astro_params::nAstroPSolved)
     {
         firstStar = static_cast<long>(lsqr_arrs::matrixIndexAstro[0] / astro_params::nAstroPSolved);
         lastStar = static_cast<long>(lsqr_arrs::matrixIndexAstro[mapNoss[system_params::myid] - 1] / astro_params::nAstroPSolved);
     ...
     //////////////////////////////////////
     auto sysmat_end_time = get_time();
     #ifdef USE_MPI
         ACTION_IF_RANK_0(system_params::myid, printf("Time to set system coefficients %lf \n", sysmat_end_time));
     #else
         printf("Time to set system coefficients %lf \n", compute_time(sysmat_end_time,sysmat_start_time));
     #endif
     ...
     std::cout<<"firstStart "<<firstStar<<" lastStar "<<lastStar<<std::endl;
     ...
     if (constr_params::extConstraint)
     {
         ...
         #ifdef USE_MPI
             ACTION_IF_RANK_0(system_params::myid, printf("Time to check nullspace: %ld\n", compute_time(end_time,start_time)));
         #else
     ...
     #else
         numberOfCompCovEle=localNumberMapStar;
     #endif
     ...
     auto precondvec_start_time = get_time();
     ...
     auto precondvec_end_time = get_time();
 
     #ifdef USE_MPI
         ACTION_IF_RANK_0(system_params::myid, printf("Time precondvector %21.10lf \n", compute_time(precondvec_end_time,precondvec_start_time)));
         MPI_Barrier(MPI_COMM_WORLD);
     ...
     #else
         printf("Time precondvector %21.10lf \n", compute_time(precondvec_end_time,precondvec_start_time));
     #endif
     ...
     /////////////// MAIN CALL
     auto precont_start_time = get_time();
     ...
     auto precond_end_time = get_time();
 
     ACTION_IF_RANK_0(system_params::myid, printf("time routing precondSystemMatrix %21.10lf \nSystem built, ready to call LSQR.\nPlease wait. System solution is underway...", compute_time(precond_end_time,precont_start_time)));
 
     startTime = get_time();
     // seconds[1] = time(NULL);
     // tot_sec[0] = seconds[1] - seconds[0];
     // comlsqr.totSec = tot_sec[0];
     auto time_before_lsqr=compute_time(startTime,initialTime);
     auto elapsed = startTime - initialTime;
     initialTime += elapsed; 
     ...
     auto start_time_depreconditioning = get_time();
     auto res=de_preconditioning(lsqr_arrs::xSolution,lsqr_arrs::standardError,lsqr_arrs::preCondVect,system_params::nunkSplit,att_params::VroffsetAttParam,astro_params::VrIdAstroPDim,system_params::myid,lsqr_input::idtest,astro_params::nAstroPSolved);
     auto end_time_depreconditioning = get_time();
     ...
     if (system_params::myid == 0)
     {
         ...
         // tot_sec[1] = start_time_depreconditioning - startTime;
         // tot_sec[2] = end_time_depreconditioning - start_time_depreconditioning;
         // tot_sec[3] = end_time_depreconditioning - initialTime;
         ...
         printf("\nEnd run.\ntime before lsqr in sec: %12.6f\ntime for lsqr: %12.6f\ntime after lsqr in sec: %12.6f\ntime TOTAL: %12.6f\n",
                time_before_lsqr, 
                compute_time(start_time_depreconditioning,startTime), 
                compute_time(end_time_depreconditioning,start_time_depreconditioning),
                compute_time(end_time_depreconditioning,initialTime));
