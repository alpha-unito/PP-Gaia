...
void initThread(struct comData *comlsqr)
{
    ...
    if (comlsqr->mapNoss[myid] % nthreads > 0)
        comlsqr->mapForThread[0][2]++;
    ...
    for (int n = 1; n < nthreads; n++)
    {
        ...
        if (comlsqr->mapNoss[myid] % nthreads > n)
            comlsqr->mapForThread[n][2]++;
    }
...
struct nullSpace cknullSpace(double *sysmatAstro, double *sysmatAtt, double *sysmatInstr, double *sysmatGloB, double *sysmatConstr, long *matrixIndexAstro, long *matrixIndexAtt, double *attNS, struct comData comlsqr)
{
    ...
    if (!nullSpaceVect)
        exit(err_malloc("nullSpaceVect", myid));
    ...
    for (int ic = 0; ic < nEqExtConstr; ic++)
    {
        ...
        {
            if (localMin > productNS[j])
                localMin = productNS[j];
            if (localMax < productNS[j])
                localMax = productNS[j];
...
int computeInstrConstr(struct comData comlsqr, double *instrCoeffConstr, int *instrColsConstr, int *instrConstrIlung)
{
    ...
    if (lsInstrFlag)
    {
        ...
        if (ssInstrFlag)
        {
            ...
            {
                for (int j = 0; j < 3; j++)
                { // each CCD generates 3 constraint equations, one for each order of the legendre polinomials
                    instrConstrIlung[counterEqs] = nElemICSS;
                    elemAcc += nElemICSS;
                    counterEqs++;
                    for (int k = 0; k < nPixelColumns; k++)
                    {
                        x = (k + 0.5) / static_cast<double>(nPixelColumns);
