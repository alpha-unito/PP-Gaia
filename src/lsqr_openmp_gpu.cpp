...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : offLocalAstro, nobs, nAstroPSolved) thread_limit(THREADSAPROD1ASTRO) // num_teams(TEAMSAPROD1)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : nAttP, offLocalAtt, nobs, nAttParAxis) thread_limit(THREADSAPROD1) // num_teams(TEAMSAPROD1)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : nobs, offLocalInstr, nInstrPSolved) thread_limit(THREADSAPROD1) // num_teams(TEAMSAPROD1)
#else
...
{
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : offLocalGlob, nobs, nGlobP) thread_limit(THREADSAPROD1) // num_teams(TEAMSAPROD1)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : nDegFreedomAtt, nobs, nEqExtConstr, nOfElextObs, numOfExtStar, numOfExtAttCol, nAstroPSolved, nAttAxes) thread_limit(TILEWIDTH) // num_teams(TEAMSAPROD1)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : nOfElBarObs, nEqBarConstr, numOfBarStar, nAstroPSolved) thread_limit(TILEWIDTH) // num_teams(TEAMSAPROD1)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : nobs, myid, nOfInstrConstr, nproc, nInstrPSolved) thread_limit(TILEWIDTH) // num_teams(TEAMSAPROD1)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : offLocalAstro, nobs, nAstroPSolved) thread_limit(THREADSAPROD2ASTRO) // num_teams(TEAMSAPROD2ASTRO)
#else
#pragma omp target teams distribute parallel for map(to : offLocalAstro, nobs, nAstroPSolved)
#endif
    for (long ix = 0; ix < nobs; ++ix)
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : offLocalAtt, nDegFreedomAtt, nAttP, nobs, nAttParAxis) thread_limit(THREADSAPROD2) // num_teams(TEAMSAPROD2)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : offLocalInstr, nobs, nInstrPSolved) thread_limit(THREADSAPROD2) // num_teams(TEAMSAPROD2)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : nEqExtConstr, nobs, nOfElextObs, numOfExtStar, numOfExtAttCol, nAstroPSolved) thread_limit(TILEWIDTH) // num_teams(TEAMSAPROD2)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : nDegFreedomAtt, nEqExtConstr, nobs, nOfElextObs, numOfExtStar, numOfExtAttCol, nAstroPSolved, nAttAxes) thread_limit(TILEWIDTH) // num_teams(TEAMSAPROD2)
#else
...
#ifndef AUTO_TUNING
#pragma omp target teams distribute parallel for map(to : nEqBarConstr, nobs, nEqExtConstr, nOfElextObs, nOfElBarObs, numOfBarStar, nAstroPSolved) thread_limit(TILEWIDTH) // num_teams(TEAMSAPROD2)
#else
...
#pragma omp target enter data map(to : sysmatAstro[0 : mapNoss * nAstroPSolved],                  \
...
#pragma omp target update to(vVect[0 : nunkSplit])
    ...
    if (wantse)
        dload(standardError, nunkSplit, ZERO);
...
#pragma omp target update to(standardError[0 : nunkSplit])
    ...
    if (!myid)
    {
        ssq_knownTerms = sumCommMultiBlock_double(knownTerms_dev, nElemKnownTerms, max_knownTerms);
    ...
    if (beta > ZERO)
    {
        ...
        if (nAstroPSolved)
            aprod2_Kernel_astro(vVect, sysmatAstro, knownTerms_dev, matrixIndexAstro, startend, offLocalAstro, nnz, nAstroPSolved);
        if (nAttP)
            aprod2_Kernel_att_AttAxis(vVect, sysmatAtt, knownTerms_dev, matrixIndexAtt, nAttP, nDegFreedomAtt, offLocalAtt, mapNoss, nAstroPSolved, nAttParAxis);
        if (nInstrPSolved)
            aprod2_Kernel_instr(vVect, sysmatInstr, knownTerms_dev, instrCol, offLocalInstr, mapNoss, nInstrPSolved);
        // APROD2 CONSTRAINTS
        if (nEqExtConstr)
            aprod2_Kernel_ExtConstr(vVect, sysmatConstr, knownTerms_dev, mapNoss, nDegFreedomAtt, VrIdAstroPDimMax, nEqExtConstr, nOfElextObs, numOfExtStar, startingAttColExtConstr, numOfExtAttCol, nAstroPSolved, nAttAxes);
        if (nEqBarConstr)
            aprod2_Kernel_BarConstr(vVect, sysmatConstr, knownTerms_dev, mapNoss, nEqBarConstr, nEqExtConstr, nOfElextObs, nOfElBarObs, numOfBarStar, nAstroPSolved);
        if (nOfInstrConstr)
            aprod2_Kernel_InstrConstr(vVect, sysmatConstr, knownTerms_dev, instrConstrIlung, instrCol, VrIdAstroPDimMax, nDegFreedomAtt, mapNoss, nEqExtConstr, nEqBarConstr, nOfElextObs, nOfElBarObs, myid, nOfInstrConstr, nproc, nInstrPSolved, nAstroPSolved, nAttAxes);
...
#pragma omp target exit data map(delete : sysmatAstro, \
                                     sysmatAtt,        \
                                     sysmatInstr,      \
                                     sysmatGloB,       \
                                     sysmatConstr,     \
...
#pragma omp target update to(vVect[0 : nunkSplit], wVect[0 : nunkSplit])
    ...
    //  ==================================================================
    if (!myid)
        std::cout << "LSQR: START ITERATIONS" << std::endl;
...
// //////////////////////////////////// APROD MODE 1
        ...
        if (nAstroPSolved)
            aprod1_Kernel_astro(knownTerms_dev, sysmatAstro, vVect, matrixIndexAstro, offLocalAstro, mapNoss, nAstroPSolved);
        ...
        if (nAttP)
            aprod1_Kernel_att_AttAxis(knownTerms_dev, sysmatAtt, vVect, matrixIndexAtt, nAttP, mapNoss, nDegFreedomAtt, offLocalAtt, nAttParAxis);
        ...
        if (nInstrPSolved)
            aprod1_Kernel_instr(knownTerms_dev, sysmatInstr, vVect, instrCol, mapNoss, offLocalInstr, nInstrPSolved);
        ...
        if (nGlobP)
            aprod1_Kernel_glob(knownTerms_dev, sysmatGloB, vVect, offLocalGlob, mapNoss, nGlobP);
        // //////////////////////////////////// CONSTRAINTS APROD MODE 1
        if (nEqExtConstr)
            aprod1_Kernel_ExtConstr(knownTerms_dev, sysmatConstr, vVect, VrIdAstroPDimMax, mapNoss, nDegFreedomAtt, startingAttColExtConstr, nEqExtConstr, nOfElextObs, numOfExtStar, numOfExtAttCol, nAstroPSolved, nAttAxes);
        if (nEqBarConstr)
            aprod1_Kernel_BarConstr(knownTerms_dev, sysmatConstr, vVect, nOfElextObs, nOfElBarObs, nEqExtConstr, mapNoss, nEqBarConstr, numOfBarStar, nAstroPSolved);
        if (nOfInstrConstr)
            aprod1_Kernel_InstrConstr(knownTerms_dev, sysmatConstr, vVect, instrConstrIlung, instrCol, VrIdAstroPDimMax, mapNoss, nDegFreedomAtt, nOfElextObs, nEqExtConstr, nOfElBarObs, nEqBarConstr, myid, nOfInstrConstr, nproc, nAstroPSolved, nAttAxes, nInstrPSolved);
#else
        if (nAstroPSolved)
            aprod1_Kernel_astro(knownTerms_dev, sysmatAstro, vVect, matrixIndexAstro, offLocalAstro, mapNoss, nAstroPSolved);
        if (nAttP)
...
//------------------------------------------------------------------------------------------------
    ...
    if (!myid)
        printf("Max Communication time: %lf \n", communicationtime);
    ...
    if (!myid)
        printf("Max Average iteration time: %lf \n", maxavtime);
#elif defined(KERNELTIME)
